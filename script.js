...
// Llama a esta función CADA VEZ que el usuario tenga una respuesta correcta
function sumarPunto() {
    let user = JSON.parse(localStorage.getItem('app_user'));
    if (user) {
        user.puntos = (user.puntos || 0) + 1;
        localStorage.setItem('app_user', JSON.stringify(user));
        
        // Enviamos la actualización al servidor en segundo plano
        syncScoreBackend(user.id, user.nombre, user.puntos);
    }
}

// Envía los datos a Google Sheets
function syncScoreBackend(id, nombre, puntos) {
    // Se usa POST para enviar los datos de forma invisible
    fetch(G_SHEETS_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `action=update&id=${id}&nombre=${nombre}&puntos=${puntos}`
    }).catch(error => console.error('Error sincronizando puntos:', error));
}

// Obtiene el Top 10 y lo dibuja en el HTML
async function cargarRanking() {
    const lista = document.getElementById('lista-ranking');
    lista.innerHTML = '<li>Cargando ranking global...</li>';
    
    try {
        const response = await fetch(G_SHEETS_URL + "?action=getRanking");
        const data = await response.json();
        
        lista.innerHTML = ''; // Limpiar
        
        if (data.length === 0) {
            lista.innerHTML = '<li>Aún no hay usuarios en el ranking.</li>';
            return;
        }

        data.forEach((user, index) => {
            // Clases CSS extra para el Top 3 (opcional, para darle estilo)
            let topClass = index < 3 ? `top-${index + 1}` : '';
            
            lista.innerHTML += `
                <li class="${topClass}">
                    <div class="rank-info">
                        <strong>#${index + 1}</strong> ${user.nombre}
                    </div>
        ...
        lista.innerHTML = '<li>Error de conexión. Intenta más tarde.</li>';
        console.error(error);
    }
}
